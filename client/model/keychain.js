...
export default class Keychain {
    constructor(secretKeyB64, nonce) {
        ...
        if (secretKeyB64) {
            this.rawSecret = strToBuffer(secretKeyB64);
        } else {
    ...
    async authKeyB64() {
        ...
        // console.log(bufferToStr(new Uint8Array(rawAuth));
        return bufferToStr(new Uint8Array(rawAuth));
    }

    async authHeader() {
        const authKey = await this.authKeyPromise;
        // console.log(await this.authKeyB64(), this.nonce)
        const sig = await crypto.subtle.sign({
                name: 'HMAC'
            },
            authKey,
            strToBuffer(this.nonce)
        );
        return `send-v1 ${bufferToStr(new Uint8Array(sig))}`;
    }
