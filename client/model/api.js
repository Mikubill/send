import { bufferToStr, strToBuffer, delay } from './utils';
...
export async function metadata(id, keychain) {
    ...
    if (result.ok) {
        ...
        const meta = await keychain.decryptMetadata(strToBuffer(data.metadata));
        return {
            size: meta.size,
            ttl: data.ttl,
            iv: meta.iv,
...
async function upload(
    ...
    try {
        ...
        const estimateSpeed = size / uploadInfo.duration * 1000
        // console.log(uploadInfo)
        // console.log(estimateSpeed)
        return uploadInfo;
    } catch (e) {
        e.size = size;
