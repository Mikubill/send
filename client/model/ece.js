...
function bufferedStream(readable) {
    let size = 64 * 1024, reader = readable.getReader(), timing;
    return new ReadableStream({
        async pull(controller) {
            ...
            let buf = [data.value], length = data.value.length;
            while (length < size) {
                if (data.done) {
                    break
                }
            ...
            controller.enqueue(Buffer.concat(buf));
        }
    })
}
...
export function encryptStream(input, key, rs = ECE_RECORD_SIZE, salt = generateSalt(KEY_LENGTH)) {
    ...
    return bufferedStream(transformStream(inputStream, new ECETransformer(mode, key, rs, salt)));
}
